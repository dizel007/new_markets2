...
// Цена за вычетом всех расходов Но с иностранными товарами (которые с отдельного отчета тянутся)
    ...
    /**************************************************************************************/
// костыль для уцененных товаров, у которых нет себестоимости 
    /**************************************************************************************/
    if (!isset($arr_real_ozon_data[$sku_ozon]['min_price'])) {
        $arr_real_ozon_data[$sku_ozon]['min_price'] = 0;
    }
...
/// Считаем все для одного товара 
    ...
    if ($arr_real_ozon_data[$sku_ozon]['count']['summa'] != 0) {
        ...
        $arr_real_ozon_data[$sku_ozon]['diff_min_price']   =   round($arr_real_ozon_data[$sku_ozon]['summa']['pribil'] / $arr_real_ozon_data[$sku_ozon]['count']['summa'], 0);
...
$nomenclatura_shop = get_catalog_tovarov_v_mp($shop_name, $pdo, '');
$nomenclatura = select_all_nomenklaturu($pdo);

foreach ($nomenclatura_shop as &$nom_shop) {
foreach ($nomenclatura as $nom) {
 if ((mb_strtolower($nom_shop['main_article']) == mb_strtolower($nom['main_article_1c']) )) {
        $nom_shop['number_in_spisok'] = $nom['number_in_spisok'];
 }
...
foreach ($arr_real_ozon_data as &$excel_item) {
    $priznak_nashli_article = 0;
    foreach ($nomenclatura_shop as $c1c_item) {
   if (isset($excel_item['mp_article'])) {
        if ((mb_strtolower($excel_item['mp_article']) == mb_strtolower($c1c_item['mp_article']) )) {
            $excel_item['number_in_spisok'] = $c1c_item['number_in_spisok'];
            $priznak_nashli_article = 1;
           break 1;
        }
     } 
...
// если не нашли артикул то присваиваем лквый
if ($priznak_nashli_article <> 1) {
    $temp = rand(0,10000);
    $excel_item['mp_article'] = "temp_art_".$temp;
    $excel_item['number_in_spisok'] = 100000000 + $temp;
    // $excel_item['product_id '] = "temp_product_id ".$temp;
...
if ($type_sort != '') {
    uasort($arr_real_ozon_data, "$type_sort");
} else {
    usort($arr_real_ozon_data, fn($a, $b) => $a['number_in_spisok'] <=> $b['number_in_spisok']);

}


// Сортировка по сумме продаж )цена продавца)
function sort_accruals_for_sale_ot_max_k_min($a, $b)
{
    return  $b['summa']['accruals_for_sale'] - $a['summa']['accruals_for_sale'];
}
function sort_accruals_for_sale_ot_min_k_max($a, $b)
{
    return  $a['summa']['accruals_for_sale'] - $b['summa']['accruals_for_sale'];
}

function sort_accruals_for_sale_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['accruals_for_sale'] - $a['one_item']['accruals_for_sale'];
}
function sort_accruals_for_sale_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['accruals_for_sale'] - $b['one_item']['accruals_for_sale'];
}

// Сортировка комиссии озон
function sort_sale_commission_ot_max_k_min($a, $b)
{
    return  $b['summa']['sale_commission'] - $a['summa']['sale_commission'];
}
function sort_sale_commission_ot_min_k_max($a, $b)
{
    return  $a['summa']['sale_commission'] - $b['summa']['sale_commission'];
}

function sort_sale_commission_one_procent_ot_max_k_min($a, $b)
{
    return  $b['one_procent']['sale_commission'] * 100 - $a['one_procent']['sale_commission'] * 100;
}
function sort_sale_commission_one_procent_ot_min_k_max($a, $b)
{
    return  $a['one_procent']['sale_commission'] * 100 - $b['one_procent']['sale_commission'] * 100;
}

function sort_sale_commission_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['sale_commission'] - $a['one_item']['sale_commission'];
}
function sort_sale_commission_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['sale_commission'] - $b['one_item']['sale_commission'];
}

// Сортировка логистике 
function sort_logistika_ot_max_k_min($a, $b)
{
    return  $b['summa']['logistika'] - $a['summa']['logistika'];
}
function sort_logistika_ot_min_k_max($a, $b)
{
    return  $a['summa']['logistika'] - $b['summa']['logistika'];
}

function sort_logistika_one_procent_ot_max_k_min($a, $b)
{
    return  $b['one_procent']['logistika'] * 100 - $a['one_procent']['logistika'] * 100;
}
function sort_logistika_one_procent_ot_min_k_max($a, $b)
{
    return  $a['one_procent']['logistika'] * 100 - $b['one_procent']['logistika'] * 100;
}

function sort_logistika_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['logistika'] - $a['one_item']['logistika'];
}
function sort_logistika_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['logistika'] - $b['one_item']['logistika'];
}


// Сортировка сумме продаж (цена озона)
function sort_amount_ot_max_k_min($a, $b)
{
    return  $b['summa']['amount'] - $a['summa']['amount'];
}
function sort_amount_ot_min_k_max($a, $b)
{
    return  $a['summa']['amount'] - $b['summa']['amount'];
}

function sort_amount_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['amount'] - $a['one_item']['amount'];
}
function sort_amount_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['amount'] - $b['one_item']['amount'];
}


// Сортировка сумме продаж сервисам
function sort_services_ot_max_k_min($a, $b)
{
    return  $b['summa']['services'] - $a['summa']['services'];
}
function sort_services_ot_min_k_max($a, $b)
{
    return  $a['summa']['services'] - $b['summa']['services'];
}

function sort_services_one_procent_ot_max_k_min($a, $b)
{
    return  $b['one_procent']['services'] * 100 - $a['one_procent']['services'] * 100;
}
function sort_services_one_procent_ot_min_k_max($a, $b)
{
    return  $a['one_procent']['services'] * 100 - $b['one_procent']['services'] * 100;
}

function sort_services_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['services'] - $a['one_item']['services'];
}
function sort_services_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['services'] - $b['one_item']['services'];
}

// Сортировка сумме продаж сервисам
function sort_ecvairing_ot_max_k_min($a, $b)
{
    return  $b['summa']['amount_ecvairing'] - $a['summa']['amount_ecvairing'];
}
function sort_ecvairing_ot_min_k_max($a, $b)
{
    return  $a['summa']['amount_ecvairing'] - $b['summa']['amount_ecvairing'];
}

function sort_ecvairing_one_procent_ot_max_k_min($a, $b)
{
    return  $b['one_procent']['ecvairing'] * 100 - $a['one_procent']['ecvairing'] * 100;
}
function sort_ecvairing_one_procent_ot_min_k_max($a, $b)
{
    return  $a['one_procent']['ecvairing'] * 100 - $b['one_procent']['ecvairing'] * 100;
}

function sort_ecvairing_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['ecvairing'] - $a['one_item']['ecvairing'];
}
function sort_ecvairing_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['ecvairing'] - $b['one_item']['ecvairing'];
}

// Сортировка цене за вычетом всего  где есть артикул
function sort_bez_vsego_gde_est_artikul_ot_max_k_min($a, $b)
{
    return  $b['summa']['bez_vsego_gde_est_artikul'] - $a['summa']['bez_vsego_gde_est_artikul'];
}
function sort_bez_vsego_gde_est_artikul_ot_min_k_max($a, $b)
{
    return  $a['summa']['bez_vsego_gde_est_artikul'] - $b['summa']['bez_vsego_gde_est_artikul'];
}

function sort_bez_vsego_gde_est_artikul_one_item_ot_max_k_min($a, $b)
{
    return  $b['one_item']['bez_vsego_gde_est_artikul'] - $a['one_item']['bez_vsego_gde_est_artikul'];
}
function sort_bez_vsego_gde_est_artikul_one_item_ot_min_k_max($a, $b)
{
    return  $a['one_item']['bez_vsego_gde_est_artikul'] - $b['one_item']['bez_vsego_gde_est_artikul'];
}

// Сортировка цене за вычетом всего 
function sort_bez_vsego_ot_max_k_min($a, $b)
{
    return  $b['summa']['bez_vsego'] - $a['summa']['bez_vsego'];
}
function sort_bez_vsego_ot_min_k_max($a, $b)
{
    return  $a['summa']['bez_vsego'] - $b['summa']['bez_vsego'];
}

// Сортировка по прибыли
function sort_pribil_ot_max_k_min($a, $b)
{
    return  $b['summa']['pribil'] - $a['summa']['pribil'];
}
function sort_pribil_ot_min_k_max($a, $b)
{
    return  $a['summa']['pribil'] - $b['summa']['pribil'];
}
