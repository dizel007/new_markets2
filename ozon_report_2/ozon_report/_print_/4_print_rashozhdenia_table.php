...
// echo "<pre>";
// print_r($arr_type_find_servives['Услуги доставки']);

// print_r($arr_for_sum_table['Услуги доставки']);
$arr_logistiki_for_print=[];

// Перебираем весь массив логистики и формируем массив для выдачи
// если появляется новый тип доставки, то он автоматом должен сюда попасть
foreach ($arr_type_find_servives['Услуги доставки'] as $type_logistik) {
    if (isset($arr_for_sum_table['Услуги доставки'][$type_logistik])) {
         $arr_logistiki_for_print['Услуги доставки/'.$type_logistik] = $arr_for_sum_table['Услуги доставки'][$type_logistik];
    }
}

// С услгами агентов нужно в ручную выбирать, то не все попадает в логистку 
// пока только три позиции
...
// Если есть логистика, то выводим строки с ней
