...
<!-- </div> -->
    ...
    <div class="product-card">
      ...
      <div class="product-info-item"><i class="fas fa-ruble-sign"></i> ИТОГО за вычетом всего FBS: <strong><?= number_format($cost_krome_logistiki, 0, '.', ' ') ?> ₽</strong></div>
     </div>
...
<!-- </div> -->
    ...
    <div class="product-card">
        <div class="product-info">
            <div class="product-info-item"><i class="fas fa-percent"></i> Комиссия Ozon FBO (<?php echo $sales_percent_fbo ?>%): <strong><?= number_format($commissionFBO, 2, '.', ' ') ?> ₽</strong></div>
            <div class="product-info-item">
                 <i class="fas fa-box"></i> Доставка до места выдачи: <strong><?= number_format($fbo_deliv_to_customer_amount, 0, '.', ' ') ?> ₽ </strong>
            </div>
       </div>
      <div class="product-info-item"><i class="fas fa-ruble-sign"></i> ИТОГО за вычетом всего FBO: <strong><?= number_format($cost_krome_logistiki_fbo, 0, '.', ' ') ?> ₽</strong></div>
    </div>
  ...
  <div class="block_1">   
    <div class="table-wrapper">
        <table class="data-table">
            <thead>
            <tr>
                <th><i class="fas fa-map-marker-alt"></i> Регион ФБС</th>
  ...
  <div class="block_2"> 
    ...
    <div class="table-wrapper">
        <table class="data-table">
            <thead>
            <tr>
                <th><i class="fas fa-map-marker-alt"></i> Регион ФБО</th>
                <th><i class="fas fa-ruble-sign"></i> проц</th>
                <th><i class="fas fa-ruble-sign"></i> Доп стоимость</th>
                <th><i class="fas fa-ruble-sign"></i> Стоимость доставки</th>
                <th><i class="fas fa-hand-holding-usd"></i> На р/с </th>
                <th><i class="fas fa-chart-line"></i> Прибыль</th>
            ...
            <tbody>
            <?php foreach ($data_by_our_volme_fbo as $print_city_fbo): ?>
                <?php
                // Доп. класс для окраски рентабельности
                $profitClass = '';
                if ($print_city_fbo['profit'] < 150) $profitClass = 'low';
                elseif ($print_city_fbo['profit'] < 250) $profitClass = 'medium';
                ?>
                <tr>
                    <td>
                        <i class="fas fa-location-dot" style="color:#3b82f6; margin-right: 8px;"></i>
                        <?= htmlspecialchars($print_city_fbo['claster_get']) ?>
                    </td>
                    <td><?= number_format($print_city_fbo['proc_k_dop_cost_k_logistike'], 0, '.', ' ') ?>%</td>
                    <td><?= number_format($print_city_fbo['dop_cost_k_logistike'], 0, '.', ' ') ?> ₽</td>
                    <td><?= number_format($print_city_fbo['cost_norm'], 0, '.', ' ') ?> ₽</td>
                    <td><?= number_format($print_city_fbo['s_logistikoi'], 0, '.', ' ') ?> ₽</td>
                    <td class="profit-cell <?= $profitClass ?>"><?= number_format($print_city_fbo['profit'], 0, '.', ' ') ?> ₽</td>
