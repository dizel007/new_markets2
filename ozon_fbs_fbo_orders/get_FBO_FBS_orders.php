...
$shop_name = "ip_zel";
// Получаем массив ФБС заказов 
$orders_FBS_ip = query_FBS_orders_from_ozon ($token_ip_zel, $client_id_ip_zel, $date_from, $date_to);
// сохраняем файл на диске, чтобы потом детелаьно ковырятся в нем и не запрашивать новый
file_put_contents($dir_for_cache."json_fbsOrders_".$shop_name.".json", json_encode($orders_FBS_ip, JSON_UNESCAPED_UNICODE));

$orders_FBS_article_ip = make_array_with_all_FBS_FBO_orders ($orders_FBS_ip);

// Получаем массив ФБO заказов 
$orders_FBO_ip = query_FBO_orders_from_ozon ($token_ip_zel, $client_id_ip_zel, $date_from, $date_to);
// сохраняем файл на диске, чтобы потом детелаьно ковырятся в нем и не запрашивать новый
file_put_contents($dir_for_cache."json_fboOrders_".$shop_name.".json", json_encode($orders_FBO_ip, JSON_UNESCAPED_UNICODE));

$orders_FBO_article_ip = make_array_with_all_FBS_FBO_orders ($orders_FBO_ip);
// формируем перечень артикулов которые были проданы
foreach ($orders_FBS_article_ip as $key=>$z) {
   $art_ar_ip[$key] = $key;
 }
foreach ($orders_FBO_article_ip as $key=>$z) {
   $art_ar_ip[$key] = $key;
}
// Привем массив артикулов в порядок (согласно порядковому нормеру)
foreach ($arr_poriadkovii_number as $key=>$number_position) {
  if (isset($art_ar_ip[$key])){ $arr_sort_ar_ip[$key] = $number_position;}
}

// Сортировка по возрастанию с сохранением ключей
asort($arr_sort_ar_ip);

print_sum_table($shop_name ,$arr_sort_ar_ip, $orders_FBS_article_ip, $orders_FBO_article_ip);
