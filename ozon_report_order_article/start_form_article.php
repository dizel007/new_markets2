...
$select_nomenclature = $_GET['article'] ?? '';

// Формируем options заранее (удобно для heredoc)
// $nomenclature_options = '<option value="">— все —</option>';
foreach ($nomenclature as $name) {
    $sel = ((string)$select_nomenclature === (string)$name) ? 'selected' : '';
    @$nomenclature_options .= "<option $sel value=\"$name\">" . htmlspecialchars($name) . "</option>";
}
