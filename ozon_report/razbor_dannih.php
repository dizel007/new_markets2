...
function make_array_sum($arr_article)
// Функция возвращает массив сумм всех массивов по ключам
{
    foreach ($arr_article as $datas) {
        foreach ($datas as $key => $dannie) {
            if (($key == 'name') || ($key == 'sku') || ($key == 'article') || ($key == 'one_shtuka') || ($key == 'one_shtuka_buyer')) {
                continue;
            }
            $arr_sum_data[$key] = @$arr_sum_data[$key] + $dannie;
        }
    }
    return $arr_sum_data;
}
